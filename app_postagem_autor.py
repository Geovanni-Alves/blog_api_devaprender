...
def token_obrigatorio(f):
    ...
    def decorated(*args, **kwargs):
        ...
        try:
            #print("testando se entrou no try")
            resultado = jwt.decode(
                token, app.config["SECRET_KEY"], algorithms=["HS256"])
            # print(resultado["id_autor"])
...
# Criar uma postagem - POST http://localhost:5000/postagem
...
def alterar_autor(id_autor):
    ...
    db.session.commit()
    return jsonify({"Mensage": "Usuário alterado com sucesso!"})
...
def excluir_autor(id_autor):
    ...
    if not autor_existente:
        return jsonify({"Mensagem": "Este autor não exite!"})
